...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    product_temp_and_neu = temperature * neutrons_emitted
    
    if temperature < 800 and neutrons_emitted > 500 and product_temp_and_neu < 500000:
        return True
    else:
        return False
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentage_value = float((generated_power/theoretical_max_power)*100)
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    reactor = temperature * neutrons_produced_per_second
    if reactor < 0.9 * threshold:
        status = 'LOW'
    elif 0.9 * threshold < reactor and reactor < 1.1 * threshold:
        status = 'NORMAL'
    else:
        status = 'DANGER'
    return status
