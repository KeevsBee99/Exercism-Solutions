"""Functions to help play and score a game of blackjack.

How to play blackjack:    https://bicyclecards.com/how-to-play/blackjack/
"Standard" playing cards: https://en.wikipedia.org/wiki/Standard_52-card_deck
"""


def value_of_card(card):
    """Determine the scoring value of a card.

    Parameters:
        card (str): The given card.

    Returns:
        int: The value of a given card.  See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 1
        3.  '2' - '10' = numerical value.
    """

    if card == 'J' or card == 'Q' or card == 'K':
        card = 10
    elif card == 'A':
        card = 1
    elif 2 <= int(card) <= 10:
        card = int(card)

    return card

def higher_card(card_one, card_two):
    """Determine which card has a higher value in the hand.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 1
        3.  '2' - '10' = numerical value.

    Returns:
        str or tuple: The resulting tuple contains both cards if they are of equal value.
    """
    temp_num_one = 0
    temp_num_two = 0
    
    if card_one == 'J' or card_one == 'Q' or card_one == 'K':
        temp_num_one = 10
    elif card_one == 'A':
        temp_num_one = 1
    elif 2 <= int(card_one) <= 10:
        temp_num_one = int(card_one)
        
    if card_two == 'J' or card_two =='Q' or card_two =='K':
        temp_num_two = 10
    elif card_two == 'A':
        temp_num_two = 1
    elif 2<= int(card_two) <= 10:
        temp_num_two = int(card_two)
    
    if temp_num_one > temp_num_two:
        bested_card = card_one
    elif temp_num_two > temp_num_one:
        bested_card = card_two
    else:
        return card_one, card_two

    return bested_card


def value_of_ace(card_one, card_two):
    
    """Calculate the most advantageous value for an upcoming ace card.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 11 (if already in hand)
        3.  '2' - '10' = numerical value.

    Returns:
        int: Either 1 or 11, which is the value of the upcoming ace card.
    """
    if card_one == 'J' or card_one == 'Q' or card_one =='K':
         card_one = 10
    elif card_one == 'A':
         card_one = 11
    elif 2 <= int(card_one) <= 10:
         card_one = int(card_one)
         
    if card_two == 'J' or card_two == 'Q' or card_two =='K':
         card_two = 10
    elif card_two == 'A':
         card_two = 11 
    elif 2 <= int(card_two) <= 10:
         card_two = int(card_two)

    ace = 11
    
    if card_one + card_two + ace > 21:
        ace = 1
    
    return ace


def is_blackjack(card_one, card_two):
    """Determine if the hand is a 'natural' or 'blackjack'.

    Parameters:
        card_one (str): First card dealt in the hand.  See below for values.
        card_two (str): Second card dealt in the hand. See below for values.

        1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
        2.  'A' (ace card) = 11 (if already in hand)
        3.  '2' - '10' = numerical value.

    Returns:
        bool: Is the hand is a blackjack (two cards worth 21).
    """
    
    ten_cards = [('A','J'), ('A','Q'),('A','K'),('A','10'),
                ('J','A'), ('Q','A'),('K','A'),('10','A')]
    
    if (card_one, card_two) in ten_cards:
        return True
        
    return False


def can_split_pairs(card_one, card_two):
    """Determine if a player can split their hand into two hands.

    Parameters:
        card_one (str): First card in the hand.
        card_two (str): Second card in the hand.

   Returns:
        bool: Can the hand be split into two pairs? (i.e. cards are of the same value).
    """
    split_pairs = ['K','Q','J','A']

    if card_one is card_two:
        return True
    elif card_one in split_pairs and card_two in split_pairs:
        return True
        
    return False
    


def can_double_down(card_one, card_two):
    
    """Determine if a blackjack player can place a double down bet.

    Parameters:
        card_one (str): First card in the hand.
        card_two (str): Second card in the hand.

    Returns:
        bool: Can the hand can be doubled down? (i.e. totals 9, 10 or 11 points).
    """
    if card_one == 'J' or card_one == 'Q' or card_one =='K':
         card_one = 10
    elif card_one == 'A':
         card_one = 1
    elif 2 <= int(card_one) <= 10:
         card_one = int(card_one)
         
    if card_two == 'J' or card_two == 'Q' or card_two =='K':
         card_two = 10
    elif card_two == 'A':
         card_two = 1
    elif 2 <= int(card_two) <= 10:
         card_two = int(card_two)
        
    sum = card_one + card_two
    if 9 <= sum <= 11:
        return True
        
    return False
